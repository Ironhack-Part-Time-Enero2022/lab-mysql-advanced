...
select * from royalties_author ORDER BY au_id;

SELECT title_id, au_id, sum_royalty
FROM (SELECT SUM(sales_royalty)
    FROM royalties_sales
	GROUP BY title_id, au_id) AS sum_royalty;



SELECT lowqty, highqty, (lowqty - highqty) AS diferencia 
FROM discounts

SELECT (price + qty) AS ventas 
FROM titles AS t
